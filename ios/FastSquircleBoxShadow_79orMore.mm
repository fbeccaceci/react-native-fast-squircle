...
//      nil);
  ...
  CGAffineTransform translation = CGAffineTransformMakeTranslation(-boundingRect.origin.x, -boundingRect.origin.y);
  
  [squirclePath applyTransform:translation];
  
  CGPathRef shadowPathAlignedWithLayer = CGPathCreateCopy(squirclePath.CGPath);
  ...
  NSNumber *cornerSmoothing)
{
  ...
  UIImage *const boxShadowImage =
      [renderer imageWithActions:^(UIGraphicsImageRendererContext *_Nonnull rendererContext) {
        ...
        renderOutsetShadows(outsetShadows, cornerRadii, layerSize, boundingRect, context, cornerSmoothing);
